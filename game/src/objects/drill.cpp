...
void Drill::draw(float dt, glm::vec2 direction, float speed, glm::vec2 pos)
{
    ...
    if (!createdFirstCart) {
        drillNode->position = (getCurrentTile(pos, direction) - direction) * 16.0f;
...
{
    glm::vec currentTile = getCurrentTile(current->position, current->position);
    if ((current->lastTile.x + 1) == currentTile.x || (current->lastTile.x == currentTile.x) && current->direction == right) {
    } else {

        bi::log("skipped tile speed is too fast, please lower below:", speed);
        //throw std::runtime_error("speed to fast for game");
        //            glm::vec2 nextTile = (current->lastTile + direction) * 16.0f;
        //            current->position = nextTile;
        //            current->lastTile = nextTile;
    }
}
